...
## Overview
The **Search & Filtering Management System** enables users to search for itineraries using keywords and multiple filters (dates, cost, location, name, etc.)

Users are able to:
- Search itineraries by Title or Description
- Filter by date, cost and location
...
- Itinerary model to store data for each trip
    - Attributes:
        ...
        - `cost:integer`
    - Methods
        - `search_by_keyword`, `filter_by_date_range`, `filter_by_location`, `filter_by_trip_type`, `filter_by_cost`
### View
- `itineraries/index.html.erb`
    - A search bar and filter form with date range, location, trip type, and min/max cost
    - Displays a list of matching itineraries
    - Shows "no itineraries found" when filters return no results
    - A "Search" button to submit form and a "Clear Filters" button
